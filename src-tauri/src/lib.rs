...
const TOOLBAR_SCRIPT: &str = r###"(() => {
  ...
  function fitHarnessBelowTitlebar() {
    ...
    for (let i = 0; i < all.length; i++) {
      const el = all[i];
      if (el === toolbar || toolbar.contains(el) || winbar && (el === winbar || winbar.contains(el)) || panel && panel.contains(el)) { continue; }
  ...
  function keepOnTop() {
    ...
    if (body && body.lastElementChild !== toolbar) { body.appendChild(toolbar); }
    if (winbar && body && body.lastElementChild !== winbar) { body.appendChild(winbar); }
  }
  function startFitting() {
    ...
    const observer = new MutationObserver(function () {
      if (pending) { return; }
      pending = true;
      setTimeout(function () { pending = false; fitHarnessBelowTitlebar(); keepOnTop(); }, 400);
    });
  ...
  function mountToolbar() {
    ...
    winbar.className = 'deepx-winbar';
    winbar.innerHTML = '<button class="deepx-win deepx-win-min" title="最小化">─</button><button class="deepx-win deepx-win-max" title="最大化">□</button><button class="deepx-win deepx-win-close" title="关闭">✕</button>';
    document.body.appendChild(winbar);
    ...
    winbar.querySelector('.deepx-win-min').addEventListener('click', function () { win('minimize'); });
    winbar.querySelector('.deepx-win-max').addEventListener('click', function () { win('toggle_maximize'); });
    winbar.querySelector('.deepx-win-close').addEventListener('click', function () { win('close'); });
    startFitting();
  }
