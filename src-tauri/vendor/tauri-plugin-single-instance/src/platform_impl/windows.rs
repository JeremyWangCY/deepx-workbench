...
use windows_sys::Win32::{
    ...
    UI::WindowsAndMessaging::{
        ...
        RegisterClassExW, SendMessageW, CREATESTRUCTW, GWLP_USERDATA, WINDOW_LONG_PTR_INDEX,
        WM_COPYDATA, WM_CREATE, WM_DESTROY, WNDCLASSEXW, WS_EX_NOACTIVATE, WS_EX_TOOLWINDOW,
        WS_OVERLAPPED,
    },
};
...
fn create_event_target_window<R: Runtime>(
    ...
    unsafe {
        ...
        // DeepX Windows fix: this target only needs to receive WM_COPYDATA from
        // another process. Keeping it as a hidden 0x0 tool window avoids creating
        // a visible layered popup at (0,0), which can leak a white DirectComposition
        // surface on some Windows/DWM configurations. FindWindowW still sees hidden
        // top-level windows, so the plugin's existing mutex + WM_COPYDATA protocol is
        // unchanged; only the desktop-composition participation is removed.
        CreateWindowExW(
